...
class PerlinNoise {
    ...
    init() {
        // Tabela de permutação original de Perlin
        this.permutation = [151,160,137,91,90,15,
            131,13,201,95,96,53,194,233,7,225,140,36,103,30,
            69,142,8,99,37,240,21,10,23,190, 6,148,247,120,234,75,0,26,
    ...
    // Função principal para calcular o ruído
    noise(x, y, z) {
        // Encontre as coordenadas da caixa que envolve o ponto
        const X = Math.floor(x) & 255;
        const Y = Math.floor(y) & 255;
        const Z = Math.floor(z) & 255;

        // Encontre o deslocamento dentro da caixa
        x -= Math.floor(x);
        y -= Math.floor(y);
        z -= Math.floor(z);

        // Calcule o fade para cada coordenada
        const u = this.fade(x);
        const v = this.fade(y);
        const w = this.fade(z);

        // Encontre os índices das cantos da caixa
        const A = this.p[X] + Y;
        const AA = this.p[A] + Z;
        ...
        // Calcule os gradientes e interpole
        ...
        return (res + 1) / 2; // Normalizar para [0,1]
    }
}
